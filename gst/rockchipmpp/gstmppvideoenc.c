...
#if 0
    for (guint i = 0;
        i < (GST_VIDEO_FORMAT_INFO_N_PLANES (self->info.finfo) - 1); i++) {
      ptr += self->info.offset[i];
      gst_buffer_extract (buffer, self->input_state->info.offset[i], ptr,
          gst_buffer_get_size (buffer)
          - self->input_state->info.offset[i]);
    }
#endif
